...
export const ERRORS: {[category: string]: {[subcategory: string] : IErrorData}} = {
    BAD_REQUEST: {
        ...
        INVALID_LOGIN: { status: HTTP_STATUS.BAD_REQUEST, code: 'INVALID_LOGIN', message: 'Incorrect username or password.' },
        INVALID_OBJECT_ID: { status: HTTP_STATUS.BAD_REQUEST, code: 'INVALID_OBJECT_ID', message: 'Given value does not match a valid mongo ObjectId.'},
    },
    UNAUTHORIZED: {
        GENERAL: { status: HTTP_STATUS.UNAUTHORIZED, code: 'UNAUTHORIZED', message: 'Unauthorized.' },
        MISSING_TOKEN: { status: HTTP_STATUS.UNAUTHORIZED, code: 'MISSING_TOKEN', message: 'Missing token in Authorization header.' },
    },
    FORBIDDEN: {
        GENERAL: { status: HTTP_STATUS.FORBIDDEN, code: 'FORBIDDEN', message: 'Forbidden.' },
        ROLE_MISMATCH: { status: HTTP_STATUS.FORBIDDEN, code: 'ROLE_MISMATCH', message: "Authenticated user's role doesn't match required role." },
    },
    NOT_FOUND: {
        ...
        COURSE_CLASS: { status: HTTP_STATUS.NOT_FOUND, code: 'COURSE_CLASS_NOT_FOUND', message: 'Course class not found.' },
    ...
    CONFLICT: {
        ...
        USER_EXISTS: { status: HTTP_STATUS.CONFLICT, code: 'USER_EXISTS', message: 'User with same email exists already.'},
    },
    INTERNAL_SERVER_ERROR: {
        GENERAL: { status: HTTP_STATUS.SERVER_ERROR, code: 'INTERNAL_SERVER_ERROR', message: 'Internal Server Error.' },
        CORRUPTED_DATABASE: {status: HTTP_STATUS.SERVER_ERROR, code: 'CORRUPTED_DATABASE', message: 'Database data is corrupted, it needs manual cleaning.'}
    },
};
