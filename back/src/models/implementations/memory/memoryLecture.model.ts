...
export default class MemoryLecture extends Lecture {
    ...
    public async getBuilding(): Promise<Building> {
        const maybeBuilding = getParentFromChild<Building>(MEMORY_DATABASE.lecturesOfBuilding, MEMORY_DATABASE.buildings, this.id);
    ...
    public async getCourseClass(): Promise<CourseClass> {
        const maybeCourseClass = getParentFromChild<CourseClass>(MEMORY_DATABASE.lecturesOfCourseClass, MEMORY_DATABASE.courseClasses, this.id);
