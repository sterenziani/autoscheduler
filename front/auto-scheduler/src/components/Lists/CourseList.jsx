    ...
    loadCourses() {
        if (this.state.user.type === 'student') {
            ApiService.getFinishedCourses(this.state.user.name).then((data) => {
                ...
                else this.setState({ courses: data, loading: false });
            });
            ...
            <React.Fragment>
                ...
                >
                    <Modal.Header closeButton>
                        <Modal.Title>
                            <Translation>{(t) => t('modal.deleteCourse')}</Translation>
                        </Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Translation>
                            {(t) =>
                                t('modal.areYouSureCourse', {
                                    code: this.state.courseToDelete.internalId,
                                    name: this.state.courseToDelete.name,
                                })
                            }
                        </Translation>
                    </Modal.Body>
                    <Modal.Footer>
                        ...
                        <Button
                            variant="danger"
                            onClick={() => {
                                this.deleteCourse(this.state.courseToDelete);
                            }}
                        >
