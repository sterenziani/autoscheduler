...
class CourseRequirementsList extends Component {
    ...
    deleteCourse() {
        if (!this.state.courseToDelete) return;
            ...
            <React.Fragment>
                <div className="pt-4">
                    {this.state.courses && this.state.courses.length > 0
                        ? [
                              this.state.courses.map((entry, index) => (
                                  <Row
                                      ...
                                      xs={1}
                                      md={4}
                                      className="px-5 mx-5 py-3 justify-content-center"
                                  >
                                      ...
                                      <div
                                          className={
                                              'my-auto w-min-50 ' + (window.innerWidth > 770 ? 'text-start' : '')
                                          }
                                      >
                                          <a className="text-white" href={'/courses/' + entry.id}>
                                              {entry.name}
                                          </a>
                                      </div>
                                  </Row>
                        ...
                        : [
                              <div key="empty-list">
                                  <Translation>{(t) => t('noRequiredCourses')}</Translation>
                              </div>,
                          ]}
                </div>
                <LinkButton
                    className="my-3"
                    variant="secondary"
                    href={'/courses/' + this.state.course.id + '/edit'}
                    textKey="edit"
                />
            </React.Fragment>
        );
