...
class SignUpStudentForm extends Component {
  ...
  register = async (values, setSubmitting, setFieldError) => {
    const { status, conflicts } = await ApiService.registerStudent(values.email, values.password, this.state.params.school, this.state.params.program)
    switch(status){
      case CREATED:
  ...
  onSubmit = (values, { setSubmitting, setFieldError }) => {
    setSubmitting(true);
    if(this.state.params.school && this.state.params.program)
            ...
            (this.state.loading)? <div className="mx-auto py-3"><Spinner animation="border"/></div> :
              ...
              <Form.Group controlId="program" className="row mx-auto form-row">
                  ...
                  <div className="col-9 text-center">
                    ...
                    {
                      (!this.state.params.program && <p key="program-error" className="form-error text-start my-0">
                      <Translation>{t => t("register.errors.school.programNotSelected")}</Translation></p>)
                    }
                  </div>
              </Form.Group>
              </>
