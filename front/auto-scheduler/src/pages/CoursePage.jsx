      ...
      if(findError)
        ...
        ApiService.getActiveUser().then((userData) => {
          ...
          if(findError)
            ...
            this.setState({ loading: false, user: userData, course: data });
        });
      }
    });
  }
        ...
        <div className="container my-5">
          <Tabs className="borderless-tabs" defaultActiveKey={"classes"} fill>
            <Tab className="text-center" eventKey="required_courses" title={<Translation>{t => t("tabs.requiredCourses")}</Translation>}>
              <div className="bg-primary rounded-bottom"><CourseRequirementsList course={this.state.course}/></div>
            </Tab>
            <Tab className="text-center" eventKey="classes" title={<Translation>{t => t("tabs.courseClasses")}</Translation>}>
              <div className="bg-primary rounded-bottom"><CourseClassesTab user={this.state.user} course={this.state.course}/></div>
            </Tab>
          </Tabs>
        </div>
      </React.Fragment>
