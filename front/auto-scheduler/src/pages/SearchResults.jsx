            ...
            if (findError) {
                ...
                data.forEach((s, idx) => {
                    tables.push(
                        this.drawTable(
                            this.getTimeTable(s),
                            Number(data[idx].earliest.split(':')[0]) - 1,
                            Number(data[idx].latest.split(':')[0]) + 1,
                            idx
                        )
                    );
                });
                        ...
                        {DAYS.map((d) => {
                            return (
                                <th key={'t-' + id + '-d-' + d} className="text-uppercase">
                                    <Translation>{(t) => t('days.' + d)}</Translation>
                                </th>
                            );
                        })}
                    </tr>
                    ...
                    {Array.from({ length: 24 }, (v, k) => k).map((h) => {
                        if (h >= earliest && h < latest) {
                            ...
                            DAYS.forEach((d) => {
                                        ...
                                        contents[d] = (
                                            ...
                                            >
                                                <div>
                                                    <div className="col">
                                                        <b>
                                                            {c.courseClass.course} - {c.courseClass.courseName}
                                                        </b>
                                ...
                                } else contents[d] = <td className="day-column text-uppercase bg-black"></td>;
                            });
                            return (
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <HelmetProvider>
                    <Helmet>
                        <title>Results - AutoScheduler</title>
                    </Helmet>
                </HelmetProvider>
                <div className="m-5 text-black">
                    <div className="d-flex justify-content-center align-items-center">
                        {this.state.scheduleIndex > 0
                            ? [
                                  <i
                                      key="back-e"
                            ...
                            : [
                                  <i
                                      key="back-d"
                                      className="bi bi-arrow-left-circle-fill btn btn-lg arrow-button-big disabled color-disabled"
                                  ></i>,
                              ]}
                        <div key={'schedule-' + this.state.scheduleIndex} className="mx-3 px-5 text-center">
                            <h1>
                                <Translation>
                                    {(t) => t('results.scheduleNumber', { value: 1 + this.state.scheduleIndex })}
                                </Translation>
                            </h1>
                            <p>
                                <Translation>
                                    {(t) =>
                                        t('results.recap', {
                                            days: this.state.schedules[this.state.scheduleIndex].days,
                                            hours: this.state.schedules[this.state.scheduleIndex].hours,
                                        })
                                    }
                                </Translation>
                                <br />
                                <Translation>
                                    {(t) =>
                                        t('results.timeRange', {
                                            earliest: this.state.schedules[this.state.scheduleIndex].earliest,
                                            latest: this.state.schedules[this.state.scheduleIndex].latest,
                                        })
                                    }
                                </Translation>
                            </p>
                        </div>
                        {this.state.scheduleIndex < this.state.schedules.length - 1
                            ? [
                                  <i
                                      key="back-e"
                    ...
                    {window.innerWidth > 1000 && (
                        <div className="container my-3 justify-content-center align-items-center">
                            <div className="table-responsive">{this.state.tables[this.state.scheduleIndex]}</div>
                        </div>
                    )}
                    ...
                    <div className="text-center">
                        <Button>
                            <Translation>{(t) => t('results.newSearch')}</Translation>
                        </Button>
                    </div>
                </div>
            </React.Fragment>
